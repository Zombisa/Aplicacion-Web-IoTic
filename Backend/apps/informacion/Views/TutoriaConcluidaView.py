...
class TutoriaConcluidaViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=['post'], url_path='tutoria_concluida')
    ...
    @action(detail=True, methods=['put'], url_path='tutoria_concluida')
    def editar_tutoria_concluida(self, request, pk):
        ...
        if verificarToken.validarRol(request) is True:
            ...
            if nueva_imagen and tutoria_concluida.image_r2 and nueva_imagen != tutoria_concluida.image_r2:
                imagen_anterior = tutoria_concluida.image_r2
            ...
            if nuevo_archivo and tutoria_concluida.file_r2 and nuevo_archivo != tutoria_concluida.file_r2:
                archivo_anterior = tutoria_concluida.file_r2
            ...
            serializer = TutoriaConcluidaSerializer(tutoria_concluida, data=data, partial=True)
            if serializer.is_valid():
                serializer.save()
    ...
    def eliminar_tutoria_concluida(self, request, pk):
        ...
        if verificarToken.validarRol(request) is True:
            ...
            if tutoria_concluida.image_r2:
                image_path = tutoria_concluida.image_r2.split("/")[-1]
                try:
            ...
            if tutoria_concluida.file_r2:
                file_path = tutoria_concluida.file_r2.split("/")[-1]
                try:
    ...
    @action(detail=False, methods=['get'], url_path='tutorias_concluidas')
